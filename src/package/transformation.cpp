...
public:
    ...
    virtual void record(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        if (triggerEvent == CardUsed && !player->hasFlag(objectName()) && player->getPhase() != Player::NotActive) {
            CardUseStruct use = data.value<CardUseStruct>();
    ...
    {
        if (huanghou->hasShownSkill(this) || huanghou->askForSkillInvoke(this, QVariant::fromValue(player))) {
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *player, QVariant &, ServerPlayer *huanghou) const
    {
...
class XiongsuanViewAsSkill : public OneCardViewAsSkill
{
public:
    XiongsuanViewAsSkill() : OneCardViewAsSkill("xiongsuan")
    {
        filter_pattern = ".!";
    }
...
class Xiongsuan : public TriggerSkill
{
public:
    Xiongsuan() : TriggerSkill("xiongsuan")
    {
        frequency = Limited;
        limit_mark = "@fierce";
        events << EventPhaseChanging;
        view_as_skill = new XiongsuanViewAsSkill;
    }
...
public:
    ...
    static void AcquireGenerals(ServerPlayer *zuoci, int n, QString reason)
    {
        ...
        if (n > 2) {
            ...
            for (int i = 1; i <= 2; i++) {
                QString general_name = room->askForGeneral(zuoci, acquired, QString(), true, reason, QVariant(), false);
                selected.append(general_name);
                acquired.removeOne(general_name);
            }
...
public:
    ...
    virtual bool ViewHas(const Player *zuoci, const QString &skill_name, const QString &flag) const
    {
        ...
        const Skill *skill = Sanguosha->getSkill(skill_name);
        if (skill == NULL || (skill->getFrequency() != Skill::Frequent && skill->getFrequency() != Skill::NotFrequent)) return false;
        QStringList huashens;
        if (!zuoci->property("Huashens").toString().isEmpty())
            huashens = zuoci->property("Huashens").toString().split("+");
...
void FlameMapCard::onUse(Room *room, const CardUseStruct &card_use) const
{
    ...
    if (Sanguosha->getCard(subcards.first())->hasFlag("flame_map")) {
        ...
        log.arg = "flamemap";
        room->sendLog(log);
        room->notifySkillInvoked(sunquan, "flamemap");
...
public:
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *lusu, QVariant &, ServerPlayer *) const
    {
        ...
        if (!room->askForUseCard(lusu, "@@haoshi_give!", "@haoshi-give", -1, Card::MethodNone)) {
...
public:
    ...
    virtual bool isEnabledAtPlay(const Player *player) const
    {
        return !player->hasUsed("ZhihengCard") && ((!player->ownSkill("zhiheng") && !player->getAcquiredSkills().contains("zhiheng"))
