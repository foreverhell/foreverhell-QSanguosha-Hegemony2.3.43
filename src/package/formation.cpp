...
public:
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *, QVariant &, ServerPlayer *dengai) const
    {
        ...
        judge.who = dengai;
        room->judge(judge);
        if (room->getCardPlace(judge.card->getEffectiveId()) == Player::PlaceJudge && judge.isGood()) {
            if (dengai->askForSkillInvoke("_tuntian", "gotofield"))
...
public:
    ...
    {
        events << GeneralShown << EventLoseSkill << DFDebut;
        relate_to_place = "deputy";
    ...
    virtual void record(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        foreach (const Skill *skill, player->getHeadSkillList()) {
        ...
        if (triggerEvent == GeneralShown) {
            ...
            if (data.toString() != "guanxing" || data.toString() != objectName()) return;
        }
        ...
        if (player->hasShownSkill(objectName()) && !(has_head_guanxing && player->hasShownGeneral1())) {
            ...
            room->handleAcquireDetachSkills(player, "-guanxing_jiangwei!", true);
        }
        return;
    }
...
public:
    ...
    virtual TriggerList triggerable(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (triggerEvent == Damaged && player->isAlive()) {
            foreach (ServerPlayer *yuji, yujis) {
                if (yuji->willBeFriendWith(player) && !yuji->isKongcheng() && yuji->getPile("sorcery").length() < 4)
    ...
    virtual bool cost(TriggerEvent triggerEvent, Room *room, ServerPlayer *, QVariant &data, ServerPlayer *ask_who) const
    {
        ...
        if (triggerEvent == Damaged) {
            ...
            const Card *to_put = room->askForCard(yuji, ".|" + suits.join(",") + "|.|hand", "@qianhuan-put", data, Card::MethodNone);
            if (to_put) {
                invoke = true;
                LogMessage log;
...
public:
    ...
    virtual QStringList triggerable(TriggerEvent triggerEvent, Room *, ServerPlayer *player, QVariant &data, ServerPlayer * &) const
    {
        ...
        if (triggerEvent == CardsMoveOneTime) {
            ...
            if ((move.reason.m_reason & CardMoveReason::S_MASK_BASIC_REASON) == CardMoveReason::S_REASON_USE) return QStringList();
    ...
    : Package("formation")
{
    ...
    insertRelatedSkills("tuntian", "#tuntian-dist");
