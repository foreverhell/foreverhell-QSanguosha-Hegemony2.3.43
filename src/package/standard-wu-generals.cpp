...
void FanjianCard::onEffect(const CardEffectStruct &effect) const
{
    ...
    int card_id = room->askForCardChosen(target, zhouyu, "h", "fanjian", false, Card::MethodGet);
...
public:
    ...
    static void Remove(ServerPlayer *zhoutai)
    {
        ...
        QList<int> buqu(zhoutai->getPile("scars"));
...
public:
    ...
    virtual bool cost(TriggerEvent, Room *room, ServerPlayer *zhoutai, QVariant &data, ServerPlayer *) const
    {
        ...
        if (zhoutai->askForSkillInvoke(this, data)) {
            ...
            if (n > 0) {
                ...
                zhoutai->addToPile("scars", card_ids);
            }
            return true;
        }
        return false;
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *zhoutai, QVariant &, ServerPlayer *) const
    {
        if (zhoutai->getHp() < 1) {
            ...
            const QList<int> &buqunew = zhoutai->getPile("scars");
            QList<int> duplicate_numbers;
...
{
public:
    BuquClear() : DetachEffectSkill("buqu", "scars")
    {
        frequency = Compulsory;
    }
...
void StandardPackage::addWuGenerals()
{
    ...
    skills << new HaoshiGiveViewAsSkill;
}
