    ...
    if (phase == Player::Draw)
        ...
        if (targets.isEmpty())
            ...
            foreach(const Card *card, targets.first()->getJudgingArea()){
                if (!Sanguosha->isProhibited(NULL, to_select, card) && !to_select->containsTrick(card->objectName()))
                    return true;
            }
...
void QiaobianAskCard::onUse(Room *room, const CardUseStruct &card_use) const
{
    ...
    if (phase == Player::Draw) {
        ...
        foreach(const Card *card, from->getJudgingArea()){
            if (!Sanguosha->isProhibited(NULL, to, card) && !to->containsTrick(card->objectName()))
                can_select = true;
            else
...
public:
    ...
    virtual QStringList triggerable(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data, ServerPlayer * &) const
    {
        ...
        if (TriggerSkill::triggerable(player) && index > 0 && player->canDiscard(player, "h") && !player->isSkipped(change.to))
            return QStringList(objectName());
        return QStringList();
    }
...
public:
    ...
    virtual int getDistanceLimit(const Player *from, const Card *card) const
    {
        ...
        if (from->hasShownSkill("duanliang"))
            return 1;
        else
            return 0;
    }
