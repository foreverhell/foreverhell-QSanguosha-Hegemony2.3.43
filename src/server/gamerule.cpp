...
void GameRule::onPhaseProceed(ServerPlayer *player) const
{
    ...
    case Player::RoundStart:{
        ...
        player->askForGeneralShow(true, !is_lord, true, !is_lord);
        break;
    }
    case Player::Start: {
        break;
...
bool GameRule::effect(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data, ServerPlayer *) const
{
    ...
    case FinishJudge: {
        ...
        if (room->getCardPlace(judge->card->getEffectiveId()) == Player::PlaceJudge) {
            if ((judge->reason != "luoshen" && judge->reason != "tuntian") || judge->isBad()) { //special rule of luoshen and tuntian
    ...
    case GeneralShown: {
        ...
        if (player->isAlive() && player->hasShownAllGenerals()) {
            if (player->getMark("CompanionEffect") > 0) {
                ...
                QString choice = room->askForChoice(player, "companion", choices.join("+"), QVariant(), "@companion-choose", "recover+draw+cancel");
                if (choice == "recover") {
            ...
            if (player->getMark("HalfMaxHpLeft") > 0) {
                ...
                log.from = player;
                room->sendLog(log);
                if (room->askForChoice(player, "halfmaxhp", "draw+cancel", QVariant(), "@halfmaxhp-choose") == "draw")
                    player->drawCards(1);
            }
        }
        if (player->isAlive() && Config.RewardTheFirstShowingPlayer && room->getTag("TheFirstToShowRewarded").isNull() && room->getScenario() == NULL) {
            room->setTag("TheFirstToShowRewarded", true);
            LogMessage log;
            log.type = "#FirstShowReward";
            log.from = player;
            room->sendLog(log);
            if (room->askForChoice(player, "firstshow", "draw+cancel", QVariant(), "@firstshow-choose") == "draw")
                player->drawCards(2);
        }
    }
