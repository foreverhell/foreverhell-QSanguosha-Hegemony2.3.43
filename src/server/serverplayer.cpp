...
void ServerPlayer::showGeneral(bool head_general, bool trigger_event, bool sendLog, bool ignore_rule)
{
    ...
    if (trigger_event) {
        Q_ASSERT(room->getThread() != NULL);
        QVariant data = (head_general ? "h" : "d");
...
bool ServerPlayer::askForGeneralShow(bool head, bool deputy, bool all, bool refusable)
{
    ...
    QString choice = room->askForChoice(this, "GameRule_AskForGeneralShow", choices.join("+"), QVariant(), "@generalshow-choose", all_choices);
